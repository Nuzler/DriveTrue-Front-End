...
const Customer = ({onSubmit,Load,setLoad}) => {
  ...
  const hour = now.getHours();
        if (hour < 8 || hour >= 22) {
        alert("Order Now Option Not Valid in this Time. Choose PickUp Later Option");
        return;
    }else{
    ...
    <div>
      ...
      <div className='flex flex-col  pt-7'>
        ...
        <div class="flex items-center mb-4">
            <input id="pickup-option-1" type="radio" name="pickupOption" value={form.now} checked={pickupOption === "now"} onChange={() => setPickupOption("now")} class="h-4 w-4 border-gray-300 focus:ring-2 focus:ring-blue-300" aria-labelledby="pickup-option-1" aria-describedby="pickup-option-1" />
        ...
        <div class="flex items-center mb-4">
            <input id="pickup-option-2" type="radio" name="pickupOption" value={form.later} checked={pickupOption === "later"} onChange={() => setPickupOption("later")} class="h-4 w-4 border-gray-300 focus:ring-2 focus:ring-blue-300" aria-labelledby="pickup-option-2" aria-describedby="pickup-option-2"/>
